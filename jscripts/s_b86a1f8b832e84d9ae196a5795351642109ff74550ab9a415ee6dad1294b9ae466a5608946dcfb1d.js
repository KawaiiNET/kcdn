...
window.fireoff=function(force=false){switch(force){case'true':case true:Pace.restart();jsonp_fire();clearInterval(mine);window.mine=setInterval("jsonp_fire();",8000);break;case'false':case false:default:break;}}
window.activeClass=function(oldid,newid,force=false){fireoff(force);if(mailbox_switch==newid&&mailbox_switch==oldid){$('#mailrefesh').html('<i class=" fa fa-refresh"></i>');}
...
function pm_box(data){if(data.all&&data.all!=null){var inbox_pm_count=0;var mti=$('#mtitle');var mail_count={inbox:0,sent:0,service:0,drafts:0,trash:0};window.reue_content='';window.reue_header='<table class="table table-inbox table-hover">'+'<tbody>';window.reue_footer='</tbody>'+'</table>';var inboxc=data.all.sort(function(a,b){var nameA=a.timestamp,nameB=b.timestamp;return nameB-nameA;});$.each(inboxc,function(a,b){if(b.type=="sent"){mail_count.sent++;};switch(mailbox_switch){case"buddies":mti.html('Mail&nbsp;<i class="fa fa-chevron-right fa-fw"></i>&nbsp;Buddy Requests');activeClass(mailbox_switch,"buddies",false);if(b.from==user.name)
{}
...
{};break;case"sent":mti.html('Mail&nbsp;<i class="fa fa-chevron-right fa-fw"></i>&nbsp;Sent');activeClass(mailbox_switch,"sent",false);if(b.type=="sent"&&b.from==user.name&&b.is_read==3)
...
jQuery('tcrmailbody').html(reue_header+reue_content+reue_footer);jQuery('#inbox_noti').html(mail_count.inbox);jQuery('#sent_noti').html(mail_count.sent);jQuery("td .timeago").timeago();});}
...
function headInject(tag,rel,href,type,title='',id='',replace=false){var l=document.createElement(tag);if(tag==='script')
{l.src=href;}
else
